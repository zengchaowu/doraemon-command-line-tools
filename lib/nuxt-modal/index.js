...
export default {
  add: async () => {
    exec("yarn add vue-js-modal@^2.0.1", function (error, stdout, stderr) {
    ...
    copy(
      path.join(__dirname, "lib", "vue-js-modal.js"),
      path.join(plugins, "vue-js-modal.js")
    );

    write(undefined, undefined, "plugins", (node) => {
      const name = "@/plugins/vue-js-modal";
      let isExist = false;
