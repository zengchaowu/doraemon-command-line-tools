...
export default {
  add: async () => {
    exec("yarn add @doraemon-design/list", function (error, stdout, stderr) {
    ...
    copy(
      path.join(__dirname, "lib", "infinite.client.js"),
      path.join(plugins, "infinite.client.js")
    );

    write(undefined, undefined, "plugins", (node) => {
      const name = "@/plugins/infinite.client";
      let isExist = false;
